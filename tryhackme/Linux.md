...
We need to be able to do basic functions like navigating to files, outputting their contents, and making files! The commands to do so are self-explanatory (once you know what they are, of course...).
...
Another great utility to learn about is the use of `grep`. The `grep` command allows us to search the contents of files for specific values that we are looking for.

For example, if we want to see everything that the IP address `81.143.211.90` has visited in a web server's access log (note that this is fictional), we can use `grep` to search for it.
